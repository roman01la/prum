...
(prum/defc render-count < prum/reactive [ref]
  [:div.stats {} "Renders: " (prum/react ref)])


(prum/defc bit < prum/static [n bit]
  (let [color (when (bit-test n bit) @core/*color)]
    (swap! *bclock-renders inc)
    [:td.bclock-bit {:style {:backgroundColor color}}]))


(prum/defc binary-clock < prum/reactive []
  (let [ts (prum/react core/*clock)
        msec (mod ts 1000)
...
#?(:cljs
   (defn mount! [mount-el]
     (prum/mount (binary-clock) mount-el)))
