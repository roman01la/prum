...
(defn periodic-refresh [period]
  #?(:cljs
          {:did-mount
           (fn [state]
             (let [react-comp (:prum/react-component state)
                   interval (js/setInterval #(prum/request-render react-comp) period)]
...
;; Using custom mixin
(prum/defc watches-count < (periodic-refresh 1000) [ref]
...
(prum/defc board-stats < prum/reactive [*board *renders]
  [:div.stats {}
   "Renders: " (prum/react *renders)
   [:br]
