...
(deftest test-derived
  ...
  (testing "user-provided ref"
    (let [*a (atom 0)
          *d (atom nil)]
      (prum/derived-atom [*a] ::key str {:ref *d})
      (is (= "0" @*d))
      (swap! *a inc)
      (is (= "1" @*d))))
  ...
  (testing "check-equals"
    (let [*a (atom 0)
      ...
      (prum/derived-atom [*a] ::key #(mod % 10) {:ref *d})
      (is (= 0 @*d))
      (is (= 1 @*resets))
      (reset! *a 1)
      (is (= 1 @*d))
    ...
    (let [*a (atom 0)
          ...
          *resets (atom 0)]
      (add-watch *d ::count-resets (fn [_ _ _ _] (swap! *resets inc)))
      (prum/derived-atom [*a] ::key #(mod % 10) {:ref *d :check-equals? false})
      (is (= 0 @*d))
      (is (= 1 @*resets))
      (reset! *a 1)
      (is (= 1 @*d))
